...
function normalizeSettlement(inputRaw: string): Settlement | null {
  ...
  // usdc.bot receipt link (ID not necessarily numeric)
  const mReceipt = input.match(/^https?:\/\/(www\.)?usdc\.bot\/e\/([^/?#]+)/i);
  if (mReceipt) return { type: "usdc_bot_receipt", value: input };
...
export default function InvoiceReceipt() {
  ...
  const statusSentence =
    rec?.status === "linked"
      ? "Proof linked"
      : rec?.status === "created"
      ? "Created (awaiting proof)"
      : rec?.status ?? "—";

  const settlementLabel = useMemo(() => {
    if (!rec?.settlement) return null;
    return rec.settlement.type === "usdc_bot_receipt" ? "Receipt (usdc.bot)" : "View tx (Basescan)";
  }, [rec?.settlement]);
  ...
  async function saveProof() {
    ...
    if (!settlement) {
      setNotice({
        type: "err",
        msg: "Paste a tx hash (0x…), a Basescan tx URL, or a usdc.bot /e/[id] link.",
      });
      return;
    }
        ...
        <main className="container">
          <div className="centerStage">
            <div className="glassCard">
              <div className="cardTitle">INVOICE</div>
          ...
          <div className="centerStage">
            <div className="glassCard">
              ...
              <div style={{ marginTop: 12 }}>
                <Link href="/invoice">
                  <button>Back to invoice.chat</button>
                </Link>
              </div>
            </div>
          </div>
  ...
  const payUrl = `https://usdc.bot/app?beneficiary=${encodeURIComponent(
    rec.payee?.address ?? ""
  )}&amount=${encodeURIComponent(rec.amount ?? "")}&memo=${encodeURIComponent(
    rec.memo ?? ""
  )}&ref=${encodeURIComponent(rec.id)}`;
...
/>
            ...
            {rec.invoiceNumber ? (
              <>
                <div className="subrow">
                  <span>Invoice #</span>
                  <span style={{ opacity: 0.9 }}>{rec.invoiceNumber}</span>
                </div>
                <div className="divider" />
              </>
            ) : null}
            ...
            {rec.dueAt ? (
              <>
                <div className="subrow">
                  <span>Due</span>
                  <span style={{ opacity: 0.9 }}>{String(rec.dueAt).slice(0, 10)}</span>
                </div>
                <div className="divider" />
              </>
            ) : null}
            ...
            {rec.payer?.address || rec.payer?.input ? (
              <>
                <div className="divider" />
                <div className="subrow">
                  <span>Payer</span>
                  <span style={{ opacity: 0.9 }}>
                    {rec.payer?.address ? (
                      <span style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }}>
                        {short(rec.payer.address)}
                      </span>
                    ) : (
                      <span>{rec.payer?.input}</span>
                    )}
                  </span>
                </div>
              </>
            ...
            <div className="subrow" style={{ alignItems: "flex-start" }}>
              ...
              <span style={{ textAlign: "right" }}>
                ...
                >
                  {rec.settlement ? "Replace proof" : "Link proof"}
                </button>
              </span>
            </div>
            ...
            <p style={{ marginTop: 12, fontSize: 12, opacity: 0.75, lineHeight: 1.4 }}>
              invoice.chat does not custody funds. Settlement occurs directly on-chain.
            </p>
          </div>
        </div>
      </main>
